...
with open ("urls/gru/gru_korrekt.txt", "r") as myfile:
...
output = codecs.open("presse/gru/gru.tsv","w","utf-8")
...
for count in range(0,length_data-1):
    ...
    meta = soup.findAll("span", class_="datum")
    for line in meta:
    ...
    head = soup.findAll("h1", class_="content__title") # ("h2")
    for line in head:
    ...
    corp = soup.findAll("div", class_="content__main") # ("h2")
    print corp
    for line in corp:
