...
contentInUrl = ["presse/pressemitteilungen/detail/"]

#contentnotInUrl = ["/seite/"]
#Extract only those URLs we are interested in:
with open ("urls/left/left.txt", "r") as myfile:
...
for dat in data:
    if any(word in dat for word in contentInUrl):
        dat = "https://www.linksfraktion.de/" + dat
...
newfile = open("urls/left/left_korrekt.txt", "w")
for ele in result:
    newfile.write(ele+'\n')
newfile.close()
